...
Fingerprint.prototype.writeAssetMap = function (destDir) {
  ...
  var mapPath = destDir + '/' + (this.assetMapPath ? this.assetMapPath : '/assets/assetMap.json');
  this.safeWrite(mapPath, JSON.stringify(toWrite, null, 2));
};
...
Fingerprint.prototype.writeRailsManifest = function(destDir) {
    ...
    var contents = new Buffer(JSON.stringify(assets, null, 2));
    var hash = this.hashFn(contents);

    var manifestPath = destDir + '/' + (this.railsManifestPath ? this.railsManifestPath : '/assets/manifest-' + hash + '.json');
    this.safeWrite(manifestPath, contents);
};
