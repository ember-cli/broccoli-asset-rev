...
Fingerprint.prototype.excludeContains = function (relativePath) {
  for (var i = 0; i < this.exclude.length; i++) {
    ...
    // preserve previous behavior which allowed assets/fonts to match assets/font/foo.ttf
    if (relativePath.substr(0, this.exclude[i].pattern.length) === this.exclude[i].pattern) {
      return true;
    }
  }
  return false;
