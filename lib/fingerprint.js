...
Fingerprint.prototype.writeAssetMap = function (destDir) {
  ...
  if (!fileName) {
    fileName = this.fingerprintAssetMap ? 'assetMap-' + this.hashFn(contents) + '.json' : 'assetMap.json';
  }

  this.safeWrite(destDir + '/assets/' + fileName, contents);
  this.assetMap['assets/' + (this.assetMapPath ? this.assetMapPath : 'assetMap.json')] = 'assets/' + fileName;
};
