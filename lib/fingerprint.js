...
function Fingerprint(inputTree, options) {
  ...
  this.manifestDest = options.manifestDest || manifestDest;
}
...
Fingerprint.prototype.writeRailsManifest = function(destDir) {
    ...
    var manifestDest = this.manifestDest.call(this, destDir, hash);
    fs.outputFileSync(manifestDest, contents);
};
...
/**
 * Return absolute path for the manifest file
 * @param {string} destDir
 * @param {sting} hash
 * @returns {*}
 */
function manifestDest(destDir, hash) {
  return path.join(destDir, 'assets', 'manifest-' + hash + '.json');
}
